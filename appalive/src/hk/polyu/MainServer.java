...
public class MainServer {
	...
	public static void main(String[] args) {
		...
		// Config.platformPath = args[1];
		// Config.sysLog = args[2];
		// Config.apkLog = args[3];
		// Config.taintFile = args[4];
		// Config.wrapperFile = args[5];
		// System.out.println("[DEBUG] apk -> " + apkPath);
		
		redirectSystemOutput();
		HashSet<String> analyzeApks = getAnalyzedApks();
		if (analyzeApks.contains(apkPath)) {
			// System.out.println("[DEBUG] -->> already analyzed, skip ...");
			return;
		}
		...
		try {
			...
			*/
			...
			if (!MABSendBroadcast.isEmpty()) {
				System.out.println("-->> (12) MAB found");
	...
	private static void redirectSystemOutput() {
		try {
			PrintStream ps = new PrintStream(new FileOutputStream(Config.sysLog, true));
			System.setOut(ps);
		} catch(Exception e) {
			e.printStackTrace();
		}
